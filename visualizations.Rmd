...
spotify_data <- spotify_data[grepl("[a-zA-Z]", spotify_data$artist_name), ]
...
library(ggplot2)
ggplot(spotify30k, aes(x = danceability, y = track_popularity)) +
...
```{r}
#genre distributions
ggplot(spotify30k, aes(x = playlist_genre, y = danceability, fill = playlist_genre)) +
  geom_boxplot() +
...
spotify30k$cluster <- kmeans_result$cluster

ggplot(spotify30k, aes(x = danceability, y = energy, color = factor(cluster))) +
...
```
```{r}
hist(spotify_data$danceability, 
     main = "Distribution of Danceability", 
     xlab = "Danceability", 
     col = "skyblue", 
     border = "white", 
